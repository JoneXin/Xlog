...
const events = require$$1__default$2["default"];
...
class Logger {
    ...
    err(msg) {
        const { filePos, lineNum } = this.getMsgPos(new Error());
        console.error(chalk__default["default"].redBright.bold.underline(filePos), chalk__default["default"].bgYellow.black(` rows: ${lineNum} `), ' ==> ', chalk__default["default"].redBright(msg) || '');
        // 根据环境自动监测日志是否保存
        if (this.dependENV) {
            process.env.NODE_ENV == 'production' ? this.logsQueue.push((cb) => __awaiter(this, void 0, void 0, function* () {
